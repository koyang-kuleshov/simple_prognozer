    ...
    def handle(self, *args, **kwargs):
        ...
        us_subdivisions = {
            "US": {
                ...
                for sub in Subdivision.objects.values('fips', 'id').
                order_by('fips').filter(country_id=1)
            }
        }

        countries_with_subdivisions_exclude_us = Subdivision.objects.\
            select_related() \
            .values('country', 'country__country') \
            .annotate(Count('country_id')) \
            .order_by('country_id') \
        ...
        countries_with_subdivisions_dict = {
            country['country__country']: {
                ...
                for sub in
                list(Subdivision.objects.filter(country_id=country['country']).
                     values('id', 'subdivision'))
            }
            for country in countries_with_subdivisions_exclude_us
        }

        all_countries_dict = {**countries_without_subdivisions,
                              **us_subdivisions,
                              **countries_with_subdivisions_dict}
        ...
        for countries_segment in list(all_countries_dict):
            ...
            for res in list(population):  # sub/country
                if res == 'null':
                    MainTable.objects.filter(
                        subdivision_id=all_countries_dict[countries_segment].
                        pop(None)) \
        ...
        with open(os.path.join(REGION_POPULATION_PATH,
                               'get_population_error.json'),
